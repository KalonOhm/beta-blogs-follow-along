  ...
  module V1
    class BlogsController < Api::V1::ApplicationController 
      def create
        ...
        render_errors(errors: result.errors.all, status: 400) and return unless result.success? 
        payload = {
      ...
      def update 
        result = Blogs::Operations.update_blog(params, @current_user) 
        render_errors(errors: result.errors.all, status: 400) and return unless result.success? 
        payload = {
          blog: BlogBlueprint.render_as_hash(result.payload),
          status: 201
      ...
      def destroy 
        ...
        blog.destroy
        render_success(payload: "Blog has been deleted", status: 200)
      end
    end
  end
end
